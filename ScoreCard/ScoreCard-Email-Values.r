...
readFile.output <- paste("V:/Shared Documents/ScoreCard/DigitalAdvertisingSpend/DigitalAdvertisingSpend-Overall.csv",sep="")
data.output <- read.csv(readFile.output, header=TRUE)
writeData(wb.master,3,data.output)
writeFile <- paste("C:/Users/euanm/Desktop/ScoreCard-Report/ScoreCard-Reference-",format(Sys.Date(),'%Y'),"Departures-",as.Date(Sys.Date(),format='%Y%m%d'),".xlsx",sep="")
...
writeFile.report <- paste("V:/Shared Documents/ScoreCard/ReportValues/ScoreCard-",format(Sys.Date(),'%Y'),"Departures-",as.Date(Sys.Date(),format='%Y%m%d'),".xlsx",sep="")
...
outMail[["subject"]] = paste("Score Card -",format(Sys.Date(),'%Y'),"Departures - ",as.Date(Sys.Date(),format='%Y%m%d'))
...
readFile.output <- paste("V:/Shared Documents/ScoreCard/Output/ScoreCard-OUTPUT-",as.Date(Sys.Date(),format='%Y%m%d'),"-NY.csv",sep="")
data.output <- read.csv(readFile.output, header=TRUE)
writeData(wb.master,2,data.output)
...
data.output <- read.csv(readFile.output, header=TRUE)
writeData(wb.master,3,data.output)
writeFile <- paste("C:/Users/euanm/Desktop/ScoreCard-Report/ScoreCard-Reference-",format(Sys.Date()+365,'%Y'),"Departures-",as.Date(Sys.Date(),format='%Y%m%d'),".xlsx",sep="")
...
writeFile.report <- paste("V:/Shared Documents/ScoreCard/ReportValues/ScoreCard-",format(Sys.Date()+365,'%Y'),"Departures-",as.Date(Sys.Date(),format='%Y%m%d'),".xlsx",sep="")
saveWorkbook(wb.master,file=writeFile.report,overwrite=TRUE)
...
outMail = OutApp$CreateItem(0)
outMail[["To"]] = paste("euanm@simpsontravel.com","eumackenzie@hotmail.co.uk", sep=";", collapse=NULL)
outMail[["subject"]] = paste("Score Card -",format(Sys.Date()+365,'%Y'),"Departures - ",as.Date(Sys.Date(),format='%Y%m%d'))
outMail[["body"]]="Please find attached the latest version of the score card."
outMail[["attachments"]]$Add(writeFile.report)
outMail$Send()
